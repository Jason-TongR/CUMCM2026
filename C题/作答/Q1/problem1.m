%% CUMCM2026 C题 问题1 —— 微网日前计划购电策略 (线性规划)
...
% 参考解(Python/HiGHS): 全天购电费 35118.60 元, 购电量 59482.70 kWh,
%   0:00与24:00储电量 8550.00 kWh —— 用于交叉验证。
clear; clc; close all;
...
Vv = [ones(1,T), ones(1,T), -ones(1,T), -ones(1,T), ...
      ...
      1, -1];
Aeq = sparse(Ii, Jj, Vv, 2*T+1, nx);
beq = [L - pv; zeros(T,1); 0];
...
% --- 校验 ---
res_bal = max(abs(p + pv + dis - L - ch - q));
fprintf('功率平衡最大残差: %.2e kW (应~0)\n', res_bal);

%% ---------------- 9. 写 result1.xlsx (按模板版式重建) ----------------
C1 = cell(145,2);
C1(1,:) = {'时间段','购电量'};
for i = 1:T
...
fout = fullfile(thisdir, 'result1.xlsx');
...
subplot(4,1,2); stairs(tt,p); hold on; stairs(tt,L); ylabel('功率(kW)');
legend('购电功率','小区负载'); grid on; title('购电功率与负载');
subplot(4,1,3); bar(tt,[ch,-dis],1,'stacked'); ylabel('功率(kW)');
legend('充电','放电(取负)'); grid on; title('储能充放电功率');
subplot(4,1,4); stairs(0:T, s); ylabel('储电量(kWh)'); xlabel('时间(10min格)');
hold on; yline(Smin,'--'); yline(Smax,'--'); grid on; title('储电量变化');
